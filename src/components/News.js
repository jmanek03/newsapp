...
const News = (props) => {
  ...
  const updateNews = async () => {
    ...
    try {
      setLoading(true);
      // Modified URL to match NewsData.io's expected parameters
      // Using 'country code' parameter correctly and simpler query
  ...
  const fetchMoreData = async () => {
    try {
      ...
      // NewsData.io API URL for pagination
        ...
        <div className='container'>
          <div className='row'>
            {articles?.map((element, index) => {
              return <div className="col-md-4" key={element.link || index}>
                <NewsItem 
